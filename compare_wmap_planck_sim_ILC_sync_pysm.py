...
import math
from sync_mixing_matrix_data_RJ import Mixingmatrix
import sys
from moments import synchrotron_moments
import moments
import os
...
dir1="pdfs/wmap_planck_pysm/sync/%s"%(model)
dir2="maps/wmap_planck_pysm/sync/%s"%(model)
...
#cmb = hp.read_map("./sim/wmap_planck_cmb_uk_RJ_nu0100p00GHz_total_nside0256.fits",field=None,verbose=False)
sync=hp.read_map("./sim/wmap_planck_sync_uk_RJ_nu0030p00GHz_total_nside0256.fits",field=None,verbose=False)
dust=hp.read_map("./sim/wmap_planck_dust_uk_RJ_nu0353p00GHz_total_nside0256.fits",field=None,verbose=False)
...
for i in range (no_bands):
    if(frequency[i]<100):
        maps3_qu[i]=hp.read_map("./sim/wmap_planck_cmb_uk_RJ_nu00%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
        maps3[i]=hp.read_map("./sim/wmap_planck_cmb_uk_RJ_nu00%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
    else:
        maps3_qu[i]=hp.read_map("./sim/wmap_planck_cmb_uk_RJ_nu0%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
        maps3[i]=hp.read_map("./sim/wmap_planck_cmb_uk_RJ_nu0%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
...
for i in range (no_bands):
    if(frequency[i]<100):
        maps1_qu[i]=hp.read_map("./sim/wmap_planck_sync_uk_RJ_nu00%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
        maps1[i]=hp.read_map("./sim/wmap_planck_sync_uk_RJ_nu00%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
    else:
        maps1_qu[i]=hp.read_map("./sim/wmap_planck_sync_uk_RJ_nu0%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
        maps1[i]=hp.read_map("./sim/wmap_planck_sync_uk_RJ_nu0%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
...
for i in range (no_bands):
    if(frequency[i]<100):
        maps2_qu[i]=hp.read_map("./sim/wmap_planck_dust_uk_RJ_nu00%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
        maps2[i]=hp.read_map("./sim/wmap_planck_dust_uk_RJ_nu00%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
    else:
        maps2_qu[i]=hp.read_map("./sim/wmap_planck_dust_uk_RJ_nu0%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
        maps2[i]=hp.read_map("./sim/wmap_planck_dust_uk_RJ_nu0%dp00GHz_total_nside0256.fits"%(frequency[i]),field=None,verbose=False)
...
###total maps
for i in range (no_bands):
    maps[i] =  noise[i] + maps[i] + maps1[i] + maps2[i] +maps4[i]
    maps_qu[i] =  noise_qu[i] + maps_qu[i] + maps1_qu[i] + maps2_qu[i] + maps4_qu[i]
...
#####
a=np.array([(frequency[i]/30.0)**(-3.00) for i in range(no_bands)])
w=np.zeros((3,no_bands))
w_qu=np.zeros((3,no_bands))
for i in range(3):
...
######PRILC method ###
cov_matrix_P=np.zeros((no_bands,no_bands))
for i in range(no_bands):
        for j in range (no_bands):
            if (i<=j):
                ##P
...
s_qu=np.zeros((3,npix))
s=np.zeros((3,npix))

sn_qu=np.zeros((3,npix))
sn=np.zeros((3,npix))

sf_qu=np.zeros((3,npix))
sf=np.zeros((3,npix))


s_P=np.zeros((3,npix))
sn_P=np.zeros((3,npix))
sf_P=np.zeros((3,npix))

s_c=np.zeros((3,npix))
sn_c=np.zeros((3,npix))
sf_c=np.zeros((3,npix))


s_pc=np.zeros((3,npix))
sn_pc=np.zeros((3,npix))
sf_pc=np.zeros((3,npix))

map_moments = np.zeros((no_bands, 3, npix),dtype=np.float64)
map_moments_qu = np.zeros((no_bands, 3, npix),dtype=np.float64)
map_moments,map_moments_qu = synchrotron_moments(bita_s)
...
for k in range (3):
	...
	sn[k]=sn[k]+np.array(noise[i,k,:])*w[k,i]
        ...
        #foreground residual 
        sf[k]=sf[k]+np.array(maps[i,k,:] + maps2[i,k,:] )*w[k,i]
        sf_qu[k]=sf_qu[k]+np.array(maps_qu[i,k,:] + maps2_qu[i,k,:])*w_qu[k,i]
        sf_P[k]=sf_P[k]+np.array(maps_qu[i,k,:] + maps2_qu[i,k,:])*w_P[k,i]
        sf_c[k]=sf_c[k]+np.array(maps[i,k,:] + maps2[i,k,:] + map_moments[i,k,:] )*wc[k,i]
        sf_pc[k]=sf_pc[k]+np.array(maps3_qu[i,k,:] + maps2_qu[i,k,:] + maps4_qu[i,k,:] + map_moments_qu[i,k,:])*wpc[k,i]



###input dust QU-EB transfer ##
sync_eb=np.zeros((3,hp.nside2npix(nside)))
alm=hp.map2alm(sync,pol=True)
for j in range(3):
        sync_eb[j]=hp.alm2map(alm[j],nside,pol=False,verbose=False)
for k in range(3):
        sf_c[k]=s_c[k] - sync_eb[k] - sn_c[k]
        sf_pc[k]=s_pc[k] - sync[k] - sn_pc[k]
...
hp.mollview(s[0],cmap=plt.cm.jet,sub = (131),title="ILC_T")
hp.mollview(s[1],cmap=plt.cm.jet,sub = (132),title="ILC_Q")
hp.mollview(s[2],cmap=plt.cm.jet,min=None,max=None,sub = (133),title="ILC_U")
plt.savefig("pdfs/wmap_planck_pysm/sync/diff_sync_ebilc.pdf")
hp.write_map("maps/wmap_planck_pysm/sync/sync_ebcilc.fits",s)
...
fig = plt.figure(figsize = (12, 8))
hp.mollview(s_P[0],cmap=plt.cm.jet,sub = (131),title="ILC_T")
hp.mollview(s_P[1],cmap=plt.cm.jet,sub = (132),title="ILC_Q")
hp.mollview(s_P[2],cmap=plt.cm.jet,min=None,max=None,sub = (133),title="ILC_U")
plt.savefig("pdfs/wmap_planck_pysm/diff_sync_pilc.pdf")
hp.write_map("maps/wmap_planck_pysm/sync/sync_pilc.fits",s_P)
"""

fig = plt.figure(figsize = (12, 8))
hp.mollview(s_c[0],cmap=plt.cm.jet,sub = (131),title="ILC_T")
hp.mollview(s_c[1],cmap=plt.cm.jet,sub = (132),title="ILC_Q")
hp.mollview(s_c[2],cmap=plt.cm.jet,min=None,max=None,sub = (133),title="ILC_U")
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/diff_sync_cilc_nc%s_ns%d.pdf"%(model,ncrr,nside))
hp.write_map("maps/wmap_planck_pysm/sync/%s/sync_cilc_nc%s_ns%d.fits"%(model,ncrr,nside),s_c*mask)


fig = plt.figure(figsize = (12, 8))
hp.mollview(s_pc[0],cmap=plt.cm.jet,sub = (131),title="ILC_T")
hp.mollview(s_pc[1],cmap=plt.cm.jet,sub = (132),title="ILC_Q")
hp.mollview(s_pc[2],min=None,max=None,cmap=plt.cm.jet,sub = (133),title="ILC_U")
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/diff_sync_pcilc_nc%s_ns%d.pdf"%(model,ncrr,nside))
hp.write_map("maps/wmap_planck_pysm/sync/%s/sync_pcilc_nc%s_ns%d.fits"%(model,ncrr,nside),s_pc*mask)

fig = plt.figure(figsize = (12, 8))
hp.mollview(sync[0]*msk,cmap=plt.cm.jet,sub = (131),title="ILC_T")
hp.mollview(sync[1]*msk,cmap=plt.cm.jet,sub = (132),title="ILC_Q")
hp.mollview(sync[2]*msk,min=None,max=None,cmap=plt.cm.jet,sub = (133),title="ILC_U")
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/input_sync_ns%d.pdf"%(model,nside))
hp.write_map("maps/wmap_planck_pysm/sync/%s/input_sync_ns%d.fits"%(model,nside),sync*mask)
...
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/diff_sync_fgrs_cilc_nc%s_ns%d.pdf"%(model,ncrr,nside))
hp.write_map("maps/wmap_planck_pysm/sync/%s/sync_fgrs_cilc_nc%s_ns%d.fits"%(model,ncrr,nside),sf_c*msk)
...
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/diff_sync_fgrs_pcilc_nc%s_ns%d.pdf"%(model,ncrr,nside))
hp.write_map("maps/wmap_planck_pysm/sync/%s/sync_fgrs_pcilc_nc%s_ns%d.fits"%(model,ncrr,nside),sf_pc*msk)
...
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/diff_sync_nrs_cilc_nc%s_ns%d.pdf"%(model,ncrr,nside))
hp.write_map("maps/wmap_planck_pysm/sync/%s/sync_nrs_cilc_nc%s_ns%d.fits"%(model,ncrr,nside),sn_c*msk)
...
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/diff_sync_nrs_pcilc_nc%s_ns%d.pdf"%(model,ncrr,nside))
hp.write_map("maps/wmap_planck_pysm/sync/%s/sync_nrs_pcilc_nc%s_ns%d.fits"%(model,ncrr,nside),sn_pc*msk)
...
plt.legend()
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/Pcilc_weights_QU_nc%s_ns%d.pdf"%(model,ncrr,nside))
np.savetxt("maps/wmap_planck_pysm/sync/%s/Pcilc_weights_QU_nc%s_ns%d.txt"%(model,ncrr,nside),wpc)
...
## for B-mode
cmb_sigma=np.std(sync[2])
nbins=100
plt.figure()
plt.title('B hist')
nx2, xbins2, ptchs = plt.hist(s[2][ind], bins=nbins, range=[-50,60])
plt.clf()
...
nx2, xbins2, ptchs = plt.hist(s_qu[2][ind], bins=nbins, range=[-50,60])
plt.clf()
width2 = xbins2[1] - xbins2[0]
...
nx2, xbins2, ptchs = plt.hist(s_P[2][ind], bins=nbins, range=[-50,60])
plt.clf()
width2 = xbins2[1] - xbins2[0]
...
nx2, xbins2, ptchs = plt.hist(sync[2][ind], bins=nbins, range=[-50,60])
plt.clf()
width2 = xbins2[1] - xbins2[0]
...
nx2, xbins2, ptchs = plt.hist(s_c[2][ind], bins=nbins, range=[-50,60])
plt.clf()
width2 = xbins2[1] - xbins2[0]
...
nx2, xbins2, ptchs = plt.hist(s_pc[2][ind], bins=nbins, range=[-50,60])
plt.clf()
width2 = xbins2[1] - xbins2[0]
...
plt.plot(xd_in,yd_in,linewidth=2,label='input')
plt.legend()
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/hist_sync_Bmode_nc%s_ns%d.pdf"%(model,ncrr,nside))
#plt.show()
np.savetxt("maps/wmap_planck_pysm/sync/%s/hist_sync_Bmode_nc%s_ns%d.txt"%(model,ncrr,nside),np.array([xd_s,yd_s,xd_qu,yd_qu,xd_p,yd_p,xd_c,yd_c,xd_pc,yd_pc,xd_in,yd_in]))
...
nx2, xbins2, ptchs = plt.hist(sf[2][ind]/cmb_sigma, bins=nbins, range=[-4,4])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_s=xbins2[0:nbins] + 0.5*width2
yd_s=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sf_qu[2][ind]/cmb_sigma, bins=nbins,range=[-4,4])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_qu=xbins2[0:nbins] + 0.5*width2
yd_qu=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sf_P[2][ind]/cmb_sigma, bins=nbins,range=[-4,4])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_p=xbins2[0:nbins] + 0.5*width2
yd_p=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sf_pc[2][ind]/cmb_sigma, bins=nbins,range=[-4,4])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_pc=xbins2[0:nbins] + 0.5*width2
yd_pc=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sf_c[2][ind]/cmb_sigma, bins=nbins,range=[-4,4])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_c=xbins2[0:nbins] + 0.5*width2
yd_c=nx2 /np.sum(nx2)
...
plt.plot(xd_pc,yd_pc,linewidth=2,label='pcILC')
plt.legend()
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/hist_fg_Bmode_nc%s_ns%d.pdf"%(model,ncrr,nside))
#plt.show()
np.savetxt("maps/wmap_planck_pysm/sync/%s/hist_fg_Bmode_nc%s_ns%d.txt"%(model,ncrr,nside),np.array([xd_s,yd_s,xd_qu,yd_qu,xd_p,yd_p,xd_c,yd_c,xd_pc,yd_pc]))
...
nx2, xbins2, ptchs = plt.hist(sn_pc[2][ind]/cmb_sigma, bins=nbins,range=[-0.4,0.4])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_pc=xbins2[0:nbins] + 0.5*width2
yd_pc=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sn_c[2][ind]/cmb_sigma, bins=nbins,range=[-0.4,0.4])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_c=xbins2[0:nbins] + 0.5*width2
yd_c=nx2 /np.sum(nx2)
...
plt.plot(xd_pc,yd_pc,linewidth=2,label='pcILC')
plt.legend()
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/hist_noise_Bmode_nc%s_ns%d.pdf"%(model,ncrr,nside))
#plt.show()
np.savetxt("maps/wmap_planck_pysm/sync/%s/hist_noise_Bmode_nc%s_ns%d.txt"%(model,ncrr,nside),np.array([xd_s,yd_s,xd_qu,yd_qu,xd_p,yd_p,xd_c,yd_c,xd_pc,yd_pc]))

#### E-mode ####
cmb_sigma=np.std(sync[1][ind])
nbins=100
plt.figure()
plt.title('E hist')
nx2, xbins2, ptchs = plt.hist(s[1][ind], bins=nbins, range=[-100,250])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_s=xbins2[0:nbins] + 0.5*width2
yd_s=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(s_qu[1][ind], bins=nbins, range=[-100,250])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_qu=xbins2[0:nbins] + 0.5*width2
yd_qu=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(s_P[1][ind], bins=nbins, range=[-100,250])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_p=xbins2[0:nbins] + 0.5*width2
yd_p=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sync[1][ind], bins=nbins, range=[-100,250])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_in=xbins2[0:nbins] + 0.5*width2
yd_in=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(s_c[1][ind], bins=nbins, range=[-100,250])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_c=xbins2[0:nbins] + 0.5*width2
yd_c=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(s_pc[1][ind], bins=nbins, range=[-100,250])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_pc=xbins2[0:nbins] + 0.5*width2
yd_pc=nx2 /np.sum(nx2)
...
plt.plot(xd_in,yd_in,linewidth=2,label='input')
plt.legend()
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/hist_sync_Emode_nc%s_ns%d.pdf"%(model,ncrr,nside))
#plt.show()
np.savetxt("maps/wmap_planck_pysm/sync/%s/hist_sync_Emode_nc%s_ns%d.txt"%(model,ncrr,nside),np.array([xd_s,yd_s,xd_qu,yd_qu,xd_p,yd_p,xd_c,yd_c,xd_pc,yd_pc,xd_in,yd_in]))
...
nx2, xbins2, ptchs = plt.hist(sf[1][ind]/cmb_sigma, bins=nbins, range=[-2,6])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_s=xbins2[0:nbins] + 0.5*width2
yd_s=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sf_qu[1][ind]/cmb_sigma, bins=nbins,range=[-2,6])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_qu=xbins2[0:nbins] + 0.5*width2
yd_qu=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sf_P[1][ind]/cmb_sigma, bins=nbins, range=[-2,6])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_p=xbins2[0:nbins] + 0.5*width2
yd_p=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sf_pc[1][ind]/cmb_sigma, bins=nbins,range=[-2,6])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_pc=xbins2[0:nbins] + 0.5*width2
yd_pc=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sf_c[1][ind]/cmb_sigma, bins=nbins,range=[-2,6])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_c=xbins2[0:nbins] + 0.5*width2
yd_c=nx2 /np.sum(nx2)
...
plt.plot(xd_pc,yd_pc,linewidth=2,label='pcILC')
plt.legend()
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/hist_fg_Emode_nc%s_ns%d.pdf"%(model,ncrr,nside))
#plt.show()
np.savetxt("maps/wmap_planck_pysm/sync/%s/hist_fg_Emode_nc%s_ns%d.txt"%(model,ncrr,nside),np.array([xd_s,yd_s,xd_qu,yd_qu,xd_p,yd_p,xd_c,yd_c,xd_pc,yd_pc]))

###noise residual####
nbins=100
nx2, xbins2, ptchs = plt.hist(sn[1][ind]/cmb_sigma, bins=nbins,range=[-0.6,0.6])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_s=xbins2[0:nbins] + 0.5*width2
yd_s=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sn_qu[1][ind]/cmb_sigma, bins=nbins,range=[-0.6,0.6])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_qu=xbins2[0:nbins] + 0.5*width2
yd_qu=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sn_P[1][ind]/cmb_sigma, bins=nbins,range=[-0.6,0.6])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_p=xbins2[0:nbins] + 0.5*width2
yd_p=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sn_pc[1][ind]/cmb_sigma, bins=nbins,range=[-0.6,0.6])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_pc=xbins2[0:nbins] + 0.5*width2
yd_pc=nx2 /np.sum(nx2)
...
nx2, xbins2, ptchs = plt.hist(sn_c[1][ind]/cmb_sigma, bins=nbins,range=[-0.6,0.6])
plt.clf()
width2 = xbins2[1] - xbins2[0]
xd_c=xbins2[0:nbins] + 0.5*width2
yd_c=nx2 /np.sum(nx2)
...
plt.plot(xd_pc,yd_pc,linewidth=2,label='pcILC')
plt.legend()
plt.savefig("pdfs/wmap_planck_pysm/sync/%s/hist_noise_Emode_nc%s_ns%d.pdf"%(model,ncrr,nside))
#plt.show()
np.savetxt("maps/wmap_planck_pysm/sync/%s/hist_noise_Emode_nc%s_ns%d.txt"%(model,ncrr,nside),np.array([xd_s,yd_s,xd_qu,yd_qu,xd_p,yd_p,xd_c,yd_c,xd_pc,yd_pc]))
